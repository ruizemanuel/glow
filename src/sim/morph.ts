...
/** Delay of a particle in the transition, always in [0, spread]. xNormB ∈ [−1, 1]. */
...
/** Smoothed local progress: 0 at p = 0 and 1 at p = 1 for any delay in [0, spread]. */
export function localProgress(p: number, delay: number, spread: number): number {
  return smoothstep(p * (1 + spread) - delay);
}

/**
 * Target of a body particle: A→B interpolation plus a cloud offset that rises and falls with sin(π·q).
 * Writes x, y, z into out[0..2].
 */
...
/** Center of the dot on a quadratic Bézier from A to B with the control point raised by `lift`. */
